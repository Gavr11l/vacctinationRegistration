...
<title><spring:message code="addVacctinationResultData.title" /></title>
<!-- Bootstrap core CSS -->
...
<body>
	...
	<div class="container">
		<div class="header clearfix">
			...
			<h3 class="text-muted">
				<spring:message code="addVacctinationResultData.title" />
			</h3>
		</div>
		...
		<form id="frm1" action="addvacresdata" method="post">
			...
			<p>
				<label class="data-label"><spring:message
						code="addVacctinationData.dose" /></label> ${command.domain.dose}
			</p>
			<p>
				<label class="data-label"><spring:message
						code="addVacctinationData.series" /></label> ${command.domain.series}
			</p>
			<p>
				<label class="data-label"><spring:message
						code="addVacctinationData.vacctinationDate" /></label>
			...
			<c:if test="${not command.addSuccessfull}">
				<p>
					<spring:bind path="command.reactionLocal">

						<label class="data-label"><spring:message
								code="reactionLocal" /></label>
				...
				<p>
					<spring:bind path="command.reactionGlobal">
						<label class="data-label"><spring:message
								code="reactionGlobal" /></label>
						<input type="text" name="${status.expression}"
							value="${status.value}">
						<c:if test="${fn:length(status.errorMessages) gt 0}">
							<div class="alert alert-danger">
				...
				<p>
					<spring:bind path="command.result">
						<label class="data-label"><spring:message code="result" /></label>
						<input type="text" name="${status.expression}"
							value="${status.value}">
						<c:if test="${fn:length(status.errorMessages) gt 0}">
							<div class="alert alert-danger">
