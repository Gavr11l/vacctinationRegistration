...
<title><spring:message
		code="addVacctinationRenouncementData.title" /></title>
<!-- Bootstrap core CSS -->
...
<body>
	...
	<div class="container">
		<div class="header clearfix">
			...
			<h3 class="text-muted">
				<spring:message code="addVacctinationRenouncementData.title" />
			</h3>
		</div>
		...
		<form id="frm1" action="addvacrdata" method="post">
			...
			<c:if test="${not command.addSuccessfull}">
				...
				<p>
					<spring:bind path="command.renouncementTime">

						<label class="data-label"><spring:message
								code="renouncementTime" /></label>
						<input type="text" name="${status.expression}"
							value="${status.value}">
						<c:if test="${fn:length(status.errorMessages) gt 0}">
							<div class="alert alert-danger">
				...
				<div class="form-group">
					<spring:bind path="command.patientParentId">
						<p>
							<label class="data-label"><spring:message
									code="patientParent" /></label> <select size="1"
								name="${status.expression}">
								<c:forEach var="parent" items="${command.patientParentList}">
									<option value="${parent.idPatientParent}"
										<c:if test="${fn:contains( status.value, parent.idPatientParent)}"> selected="selected"</c:if>>
										${parent.lastName} ${parent.firstName} ${parent.middleName}</option>
								</c:forEach>
							</select>
						</p>
					</spring:bind>
